...
* Check all your connections and spellings. Be very aware of whitespaces when you copy text.
...
In this lab you will run a simple Python script that simulates IoT sensor data from some hypothetical machines, and send the data to a MQTT broker (link:https://mosquitto.org/[mosquitto]). The MQTT broker plays the role of a 

that is connected to many and different type of sensors through the "mqtt" protocol. Your cluster comes with an embedded MQTT broker that the simulation script publishes to. For convenience, we will use NiFi to run the script rather than Shell commands.
...
. The only thing that remains to be configured now is to finally connect the "*ExecuteProcess*" processor to the flow in the "*Processor Sensor Data*" group. To do that, first go back to the root canvas by clicking on the *NiFi Flow* link on the status bar.
+
image::images/breadcrumbs.png[width=400]

. Connect the"*ExecuteProcess*" processor to the *Process Sensor Data* Process Group by dragging from the processor to the Process Group. When prompted, ensure the "To input" fields is set to the *Sensor data* Input Port. 
+
